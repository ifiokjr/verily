...
void main() {
  group('FaceDetectionService', () {
    ...
    setUp(() {
      ...
      when(
        mockFormat.raw,
      ).thenReturn(17); // Using Android's NV21 value as placeholder
    ...
    test(
      'processImage should emit smile gesture when smilingProbability is high',
      () async {
        final smilingFace = _createMockFace(smilingProbability: 0.9);
        when(
          mockDetector.processImage(any),
    ...
    test(
      'processImage should emit blink gesture when both eyes are closed',
      () async {
        final blinkingFace = _createMockFace(
    ...
    test(
      'processImage should emit wink gesture when only left eye is closed',
      () async {
        final winkingFace = _createMockFace(
          leftEyeOpenProbability: 0.05,
    ...
    test(
      'processImage should emit wink gesture when only right eye is closed',
      () async {
        final winkingFace = _createMockFace(
    ...
    test(
      'processImage should emit openMouth gesture based on refined logic',
      () async {
        ...
        expectLater(
          ...
          emits(
            predicate<List<FacialGesture>>((gestures) {
              final openMouthGesture = gestures.firstWhere(
                (g) => g.type == GestureType.openMouth,
                orElse:
                    () => FacialGesture(
    ...
    test(
      'processImage should emit frown gesture based on refined logic',
      () async {
        ...
        final frowningFace = _createMockFace(
          landmarks: {
            ...
            FaceLandmarkType.leftEye: _createLandmark(
              290,
              240,
            ), // Needed for height estimate
            FaceLandmarkType.rightEye: _createLandmark(350, 240),
            FaceLandmarkType.bottomMouth: _createLandmark(
              320,
              290,
            ), // Needed for height estimate
          },
        ...
        expectLater(
          ...
          emits(
            predicate<List<FacialGesture>>((gestures) {
              final frownGesture = gestures.firstWhere(
                (g) => g.type == GestureType.frown,
                orElse:
                    () => FacialGesture(
                      type: GestureType.smile,
                      confidence: -1,
    ...
    test(
      'processImage handles face with null probabilities/landmarks',
      () async {
        final nullFace = _createMockFace(); // No probabilities or landmarks
        when(
          mockDetector.processImage(any),
        ).thenAnswer((_) async => [nullFace]);
